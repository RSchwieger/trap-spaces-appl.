...
k = [6.213,6.213,6.213] # Hill coefficients
theta = [0.5, 0.5, 0.5] # thresholds
d = [1,1,1] # diagonal of D-Matrix
x0_0 = 1.0;
x0 = [0.5,0.5,0.5] # initial value (1,*,*,*)
...
def multivariateInterpolation(x):
    ...
    y[1-1] = x0_0*x[3-1]
    y[2-1] = x0_0*(1-x[3-1])
    y[3-1] = 1-x[2-1]+(1-x0_0)*x[1-1]*x[2-1]*x[3-1]
    """
    x0 = x0_0
    y[1-1] = x0*x[3-1]
    y[2-1] = (1-x0)*x[3-1]
...
plt.xlabel('time')
plt.legend(loc=0)
plt.title("initial state "+str(x0)+", d = "+str(d)+" ,theta = "+str(theta)+", k="+str(k))
